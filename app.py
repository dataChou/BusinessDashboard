...
if "_doctor_filter" not in st.session_state:
    st.session_state._doctor_filter = st.session_state.doctor_filter
if "_service_filter" not in st.session_state:
    st.session_state._service_filter = st.session_state.service_filter
...
customer_data['分层'] = customer_data['R'].apply(lambda r: '高价值客户' if r<=30 else ('活跃客户' if r<=90 else ('沉睡客户' if r<=180 else '流失客户')))
...
lost_rate = lost_customers / total_customers if total_customers else 0
avg_ticket = total_revenue / total_orders if total_orders else 0

# 服务项目
service_stats = filtered_df.groupby('服务项目').agg(总金额=('消费金额','sum'),订单数=('宠主ID','count'),客单价=('消费金额','mean')).round(2).sort_values('总金额',ascending=False)
if not service_stats.empty:
    service_stats['金额占比'] = (service_stats['总金额']/service_stats['总金额'].sum()*100).round(1)

# 医生绩效
doctor_stats = filtered_df.groupby('医生').agg(接诊量=('宠主ID','count'),总业绩=('消费金额','sum'),客单价=('消费金额','mean')).round(2).sort_values('接诊量',ascending=False)

# 月度趋势
filtered_df['月份'] = filtered_df['日期'].dt.to_period('M').astype(str)
monthly = filtered_df.groupby('月份').agg(营收=('消费金额','sum'),订单数=('宠主ID','count')).reset_index()

# UI
st.title("🐾 宠物医院经营分析看板")
st.caption(f"数据更新时间：{datetime.now().strftime('%Y-%m-%d %H:%M')}")

if len(st.session_state.doctor_filter) < len(df_raw['医生'].unique()) or len(st.session_state.service_filter) < len(df_raw['服务项目'].unique()):
    st.info(f"📌 当前筛选模式：医生={st.session_state.doctor_filter}, 服务={st.session_state.service_filter}")

col1, col2, col3, col4, col5 = st.columns(5)
with col1: st.metric("🐕 总客户数", f"{total_customers:,}")
with col2: st.metric("💰 总营收", f"{total_revenue/10000:.1f}万")
with col3: st.metric("⚠️ 流失率", f"{lost_rate*100:.1f}%")
with col4: st.metric("💳 客单价", f"{avg_ticket:.0f}元")
with col5: st.metric("📋 总订单数", f"{total_orders:,}")
st.divider()
...
with col1:
    ...
    fig = px.pie(values=layer_counts.values, names=layer_counts.index, title="客户分层占比", color_discrete_sequence=['#2ECC71','#3498DB','#F39C12','#E74C3C'])
    fig.update_traces(textposition='inside', textinfo='percent+label')
    st.plotly_chart(fig, use_container_width=True)
with col2:
    st.subheader("🏥 服务项目盈利分析")
    if not service_stats.empty:
        fig = px.bar(service_stats, x=service_stats.index, y='总金额', title="各项目营收（元）", color=service_stats.index, color_discrete_sequence=['#1ABC9C','#2ECC71','#3498DB','#9B59B6','#E74C3C'])
        st.plotly_chart(fig, use_container_width=True)
st.divider()
...
with col1:
    ...
    if not doctor_stats.empty:
        fig = px.bar(doctor_stats, x=doctor_stats.index, y='接诊量', title="医生接诊量排行", color=doctor_stats.index, color_discrete_sequence=px.colors.sequential.Blues_r)
        st.plotly_chart(fig, use_container_width=True)
with col2:
    st.subheader("💰 各项目客单价对比")
    item_price = filtered_df.groupby('服务项目')['消费金额'].mean().round(0).sort_values()
    if not item_price.empty:
        fig = px.bar(x=item_price.values, y=item_price.index, orientation='h', title="服务项目客单价（元）", color=item_price.values, color_continuous_scale='Viridis')
        st.plotly_chart(fig, use_container_width=True)
st.divider()
...
if not monthly.empty:
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=monthly['月份'], y=monthly['营收'], mode='lines+markers', name='营收', line=dict(color='#2ECC71', width=3)))
    fig.add_trace(go.Bar(x=monthly['月份'], y=monthly['订单数'], name='订单数', yaxis='y2', marker_color='#3498DB', opacity=0.6))
