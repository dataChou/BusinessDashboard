...
# ========== 关键修复部分 ==========
# 使用 agg() 方法并直接命名新列，确保 doctor_stats 只有三列
...
with col2:
    ...
    fig = px.bar(service_stats, x=service_stats.index, y='总金额', title="各项目营收（元）",
                 color=service_stats.index, color_discrete_sequence=['#1ABC9C', '#2ECC71', '#3498DB', '#9B59B6', '#E74C3C'])
    st.plotly_chart(fig, use_container_width=True)
...
fig.update_layout(title='月度营收与订单数趋势', xaxis_title='月份', yaxis_title='营收（元）',
